  ...
  .comments
    .box
      ...
      .new-comment-wrapper(v-if="newCommentMode")
        ...
        p.micetype *Please don't be a jerk.  We have enough toxicity in comp!
      hr
      p(v-if="!comments.length") There are currently no comments for this article!
      comment(v-for="comment in comments" :comment="comment" :isOwner="isOwner" :userData="userData" :key="comment.postDate" commentType="article")
        figure.media-left
          p.image.is-64x64.avatar
...
export default {
  ...
  props: ['userData', 'articleId', 'isOwner', 'verified'],
  data () {
    return {
  ...
  methods: {
    saveComment: function () {
      ...
      _this.newComment.articleId = _this.articleId
      _this.newComment.userId = _this.userData.id
      _this.newComment.teamLogo = _this.userData.teamLogo || null
      fireComment.createArticleComment(_this.newComment).then(function () {
    ...
    getTheComments: function (articleId) {
      var _this = this
      fireComment.getArticleComments(_this.articleId).then(function (comments) {
