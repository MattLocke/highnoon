  ...
  .article
    .container
      section.section
        .box(v-for="article in articles" v-if="article.approved || canApproveNews")
          .news-preview(v-bind:class="{'preview-off': article.seeFull, 'preview-on': !article.seeFull}")
            h1.text-border(v-if="article.headline")
              a(v-bind:href="articleUrl(article.id)" target="_blank") {{ article.headline }}
        ...
        comments(:userData = "userData" :articleId = "articleId" :isOwner = "isAdmin" :verified="user.emailVerified")
</template>
